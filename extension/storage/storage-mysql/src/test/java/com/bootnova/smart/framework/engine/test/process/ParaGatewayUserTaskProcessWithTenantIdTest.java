...
@Transactional
public class ParaGatewayUserTaskProcessWithTenantIdTest extends DatabaseBaseTestCase {
    ...
    public void passed() throws Exception {
        ...
        ProcessDefinition processDefinition = repositoryCommandService
            .deploy("parallel-gateway-usertask-process.bpmn.xml",tenantId).getFirstProcessDefinition();

        //4.启动流程实例
        ProcessInstance processInstance = processCommandService.start(processDefinition.getId(),"3.0.0",tenantId);
        Assert.assertNotNull(processInstance);

        List<TaskInstance> submitTaskInstanceList=  taskQueryService.findAllPendingTaskList(processInstance.getInstanceId(),tenantId);
        ...
        ProcessInstance finalProcessInstance = processQueryService.findById(processInstance.getInstanceId(),tenantId);
        ...
        finalProcessInstance = processQueryService.findById(processInstance.getInstanceId(),tenantId);
